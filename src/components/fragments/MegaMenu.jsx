...
const ActiveItem = ({ item, color, onClose, compact }) => (
  <Link
    ...
    style={{
      ...
      padding: compact ? '10px 8px' : '7px 8px',
      borderRadius: 8, textDecoration: 'none',
      color: palette.text.secondary, transition: 'background 0.15s, color 0.15s',
      minHeight: compact ? 44 : 'auto',
    }}
  ...
  >
    <span style={{ width: 28, height: 28, borderRadius: 7, background: `${color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <i className={`bi ${item.icon}`} style={{ fontSize: '0.82rem', color }} />
    </span>
...
const SoonItem = ({ item, compact }) => (
  <div style={{
    ...
    <span style={{ width: 28, height: 28, borderRadius: 7, background: palette.surface.overlay, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <i className={`bi ${item.icon}`} style={{ fontSize: '0.82rem', color: palette.text.muted }} />
    </span>
    <span style={{ fontSize: compact ? '0.88rem' : '0.84rem', color: palette.text.muted, flex: 1, lineHeight: 1.2 }}>{item.name}</span>
    <span style={{ fontSize: '0.6rem', fontWeight: 700, padding: '2px 6px', borderRadius: 20, background: palette.indigo[50], color: palette.indigo[500], whiteSpace: 'nowrap', flexShrink: 0 }}>
      Soon
    </span>
  </div>
);
    ...
    <div style={{ background: palette.surface.base, borderRadius: 14, border: `1px solid ${palette.border.default}`, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: compact ? '12px 14px 10px' : '14px 16px 10px', borderBottom: `1px solid ${palette.border.default}`, background: `${color}08`, display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ width: 36, height: 36, borderRadius: 10, background: `${color}20`, border: `1.5px solid ${color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <i className={`bi ${cat.icon}`} style={{ fontSize: '1rem', color }} />
        </span>
        <span style={{ fontWeight: 700, fontSize: compact ? '0.9rem' : '0.85rem', color: palette.text.primary, lineHeight: 1.2 }}>
          {cat.title}
        </span>
      </div>
      {/* Items */}
...
const MegaMenu = ({ categories, onClose }) => {
  ...
  /* Page scrollbar is globally hidden via CSS (html/body scrollbar-width:none).
     overscroll-behavior:contain on .mega-scroll stops the page scrolling through
     when the menu reaches its scroll end — no overflow:hidden on body needed. */
  ...
  return (
    <>
      ...
      {/*
        Panel — flex column so the header + accent bar live OUTSIDE the scroll zone.
        Only the inner content div scrolls; the header never moves.
      */}
      <div style={{
        position: 'fixed', top: 56, left: 0, right: 0, zIndex: 1041,
        ...
        <div style={{
          ...
          padding: compact ? '12px 16px' : '16px 28px 14px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ width: compact ? 30 : 34, height: compact ? 30 : 34, borderRadius: 9, background: palette.gradient.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <i className="bi bi-grid-fill" style={{ color: 'white', fontSize: compact ? '0.8rem' : '0.9rem' }} />
            </span>
            <div>
          ...
          <button
            onClick={onClose}
            style={{ width: 36, height: 36, borderRadius: '50%', border: `1px solid ${palette.border.strong}`, background: 'transparent', color: palette.text.muted, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', flexShrink: 0 }}
        ...
        {/* ── Scrollable content (cards + footer) ── */}
        <div className="mega-scroll" style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          ...
          <div style={{ padding: pad, maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: compact ? 10 : 16 }}>
              {visible.map((cat, ci) => (
                <CategoryCard key={ci} cat={cat} onClose={onClose} compact={compact} />
              ))}
            </div>
          </div>
          ...
          <div style={{ padding: compact ? '10px 16px 12px' : '10px 28px 14px', borderTop: `1px solid ${palette.border.default}`, background: palette.surface.base, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <i className="bi bi-rocket-takeoff" style={{ color: palette.indigo[400], fontSize: '0.8rem' }} />
