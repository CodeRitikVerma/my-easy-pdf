...
const CameraToPDFPage = () => {
  ...
  /* Body scroll lock */
  useEffect(() => {
    document.body.classList.toggle('camera-open', isCameraActive);
    return () => document.body.classList.remove('camera-open');
  }, [isCameraActive]);

  /* Attach stream to <video> after overlay mounts */
  useEffect(() => {
  ...
  const capturePhoto = () => {
    ...
    setTimeout(() => { captureGuardRef.current = false; }, 700);
  ...
  return (
    <>
      <SEO canonical="/camera-to-pdf" title="Camera to PDF — Mobile PDF Scanner"
        description="Use your phone camera to capture photos and instantly convert them to a PDF. No app download needed. Works directly in your mobile browser."
        keywords="camera to PDF, phone camera PDF, mobile PDF scanner, scan document to PDF, photo to PDF mobile" />
      ...
      {isCameraActive && (
        <CameraOverlay
          videoRef={videoRef} pendingPhoto={pendingPhoto} photos={photos}
          flashActive={flashActive} showGrid={showGrid} setShowGrid={setShowGrid}
          capturePhoto={capturePhoto} keepPhoto={keepPhoto} retakePhoto={retakePhoto} stopCamera={stopCamera}
        />
      )}
      ...
      <Container className="py-4" style={{ maxWidth: 640 }}>
        ...
        {photos.length > 0 && (
          <>
            ...
            <Button variant="outline-primary" className="w-100 mb-4" onClick={startCamera}>
              <i className="bi bi-camera me-2"></i>Take More Photos
            </Button>
            ...
            <Button variant="primary" size="lg" onClick={createPDF} disabled={isProcessing} className="px-4 w-100">
              {isProcessing
                ...
                : <><i className="bi bi-file-earmark-arrow-down me-2"></i>{pdfName.trim() ? `Download "${pdfName.trim()}.pdf"` : 'Create & Download PDF'} ({photos.length} page{photos.length > 1 ? 's' : ''})</>}
            </Button>
          </>
        )}
...
export default CameraToPDFPage;
