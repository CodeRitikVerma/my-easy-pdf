...
/* ── Preset colour palette (5 rows × 6 cols = 30) ── */
const PRESET_COLORS = [
  /* Neutrals */   '#000000','#1f2937','#374151','#6b7280','#9ca3af','#f9fafb',
  /* Reds/Pinks */ '#7f1d1d','#dc2626','#f87171','#ec4899','#be185d','#fda4af',
  /* Oranges */    '#78350f','#b45309','#d97706','#f59e0b','#fb923c','#fef08a',
  /* Greens */     '#14532d','#15803d','#16a34a','#22c55e','#34d399','#a7f3d0',
  /* Blues */      '#1e3a8a','#1d4ed8','#3b82f6','#60a5fa','#38bdf8','#bae6fd',
];
/* Colours light enough to need a dark checkmark */
const LIGHT_SWATCHES = new Set(['#f9fafb','#fda4af','#fef08a','#a7f3d0','#bae6fd','#9ca3af','#fb923c','#f59e0b']);
/* Relative luminance — decide text colour on the preview swatch */
const getLum = (h) => {
...
/* ── Colour-picker modal (portal → renders in document.body) ── */
const ColorPickerModal = ({ customColor, colorHex, onChange, onClose, onClear }) => {
  ...
  useEffect(() => {
    const v = customColor || colorHex;
    setHexInput(v); setPreview(v);
  ...
  const panel = (
    <div style={{
      ...
      /* Hard cap so it always fits in viewport on any screen size */
      maxHeight: isMobile ? '85vh' : 'min(520px, calc(100vh - 48px))',
      display: 'flex', flexDirection: 'column',
      boxShadow: '0 20px 60px rgba(15,23,42,0.28), 0 4px 20px rgba(15,23,42,0.10)',
    }}>
      ...
      <div style={{
        ...
        <span style={{
          ...
          <i className="bi bi-palette-fill" style={{ color: '#fff', fontSize: '0.85rem' }} />
        </span>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: '0.88rem', color: palette.text.primary, lineHeight: 1.2 }}>
            Choose Ink Color
          </div>
          <div style={{ fontSize: '0.68rem', color: palette.text.muted, marginTop: 1 }}>
            Pick a preset or enter a hex value
          </div>
        </div>
        <button onClick={onClose} style={{
          width: 30, height: 30, borderRadius: '50%',
          border: `1.5px solid ${palette.border.strong}`,
          background: 'transparent', color: palette.text.muted,
      ...
      <div style={{ overflowY: 'auto', flex: 1 }} className="thin-scroll">
        ...
        {/* Live colour preview — click to open native picker */}
        <div style={{ padding: '10px 14px 0' }}>
          <label style={{ display: 'block', cursor: 'pointer', position: 'relative', borderRadius: 10, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.10)' }}>
            <div style={{
              height: 54,
              background: validHex ? preview : '#000',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'background 0.12s',
            }}>
              <span style={{
                display: 'flex', alignItems: 'center', gap: 6,
                fontSize: '0.72rem', fontWeight: 600, color: onPrev,
                background: lum > 0.58 ? 'rgba(0,0,0,0.09)' : 'rgba(255,255,255,0.16)',
                padding: '4px 12px', borderRadius: 20, backdropFilter: 'blur(4px)',
              }}>
                <i className="bi bi-eyedropper" />Tap to open colour picker
              </span>
            </div>
            <input type="color" value={validHex ? preview : '#000000'}
        ...
        <div style={{ padding: '10px 14px 0' }}>
          <div style={{ fontSize: '0.64rem', fontWeight: 700, color: palette.text.muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 5 }}>
            <i className="bi bi-palette2" style={{ fontSize: '0.68rem', color: palette.primary }} />Quick Colors
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 6 }}>
            {PRESET_COLORS.map((c, i) => {
              const active  = preview === c;
              const isLight = LIGHT_SWATCHES.has(c);
              return (
                <button key={i} onClick={() => pick(c)} title={c} style={{
                  aspectRatio: '1', borderRadius: 8, background: c, outline: 'none',
                  border: active ? `2.5px solid ${palette.primary}` : isLight ? `1.5px solid ${palette.border.strong}` : '1.5px solid transparent',
                  cursor: 'pointer', transition: 'all 0.13s', position: 'relative',
                  boxShadow: active ? `0 0 0 2px #fff, 0 0 0 4px ${palette.primary}` : '0 1px 3px rgba(0,0,0,0.15)',
        ...
        <div style={{ padding: '10px 14px 12px' }}>
          <div style={{ fontSize: '0.64rem', fontWeight: 700, color: palette.text.muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 5 }}>
            <i className="bi bi-hash" style={{ fontSize: '0.7rem', color: palette.primary }} />Hex Value
          </div>
          <div style={{
            display: 'flex', alignItems: 'center', gap: 8,
            padding: '8px 10px',
            background: palette.surface.raised,
            borderRadius: 10, border: `1.5px solid ${palette.border.default}`,
          }}>
            <div style={{
            ...
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: palette.text.muted, flexShrink: 0 }}>#</span>
            <input
              type="text"
              ...
              style={{
                ...
                outline: 'none', color: palette.text.primary, letterSpacing: '0.06em',
              }}
            />
            <span style={{
              fontSize: '0.6rem', fontWeight: 700, padding: '2px 6px', borderRadius: 5, flexShrink: 0,
              background: `${palette.primary}15`, color: palette.primary, letterSpacing: '0.04em',
            }}>HEX</span>
          </div>
        </div>
      ...
      <div style={{
        ...
        {customColor && (
          <button onClick={onClear} style={{
            ...
            background: 'none', border: `1.5px solid ${palette.border.strong}`,
            color: palette.text.muted, fontSize: '0.82rem',
        ...
        <button onClick={onClose} style={{
          ...
          background: palette.gradient.primary, color: '#fff', border: 'none',
  ...
  return createPortal(
    <>
      {/* Backdrop */}
      <div className="cpicker-backdrop" onClick={onClose} style={{
        position: 'fixed', inset: 0, zIndex: 2000,
        background: 'rgba(15,23,42,0.55)',
        backdropFilter: 'blur(5px)', WebkitBackdropFilter: 'blur(5px)',
      }} />
      {/* Panel */}
      <div className={isMobile ? 'cpicker-panel-mobile' : 'cpicker-panel-desktop'} style={{
...
const SignPDFPage = () => {
  ...
  /* Render preview page whenever previewPage changes */
  useEffect(() => {
    if (!pdfjsDocRef.current) return;
    renderPage(pdfjsDocRef.current, previewPage + 1, 1.5)
      .then(setPreviewPageUrl).catch(() => {});
  }, [previewPage]);

  /* Save current page transform then switch */
  const switchPage = useCallback((idx) => {
    if (sigTransformRef.current) {
      pageTransformsRef.current[previewPage] = { ...sigTransformRef.current };
      setPositionedPages(prev => new Set([...prev, previewPage]));
    }
    setPreviewPage(idx);
  }, [previewPage]);
  ...
  const handleFiles = async (files) => {
    ...
    pageTransformsRef.current = {};
    setPositionedPages(new Set());
    setIsLoading(true);
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++)
        thumbs.push({ index: i, thumbnail: await renderPage(pdf, i, 0.4) });
      setPages(thumbs);
  ...
  const handleReset = () => {
    setPdfFile(null); setPages([]); setPreviewPageUrl('');
    pdfjsDocRef.current = null; sigTransformRef.current = null;
    pageTransformsRef.current = {};
    setPositionedPages(new Set());
  };
  ...
  const togglePage = (idx) => {
    setSelectedPages(prev => {
      ...
      if (next.has(idx) && next.size > 1) next.delete(idx); else next.add(idx);
      return next;
    });
    switchPage(idx);
  };
  ...
  const applySignature = async () => {
    ...
    try {
      ...
      for (const idx of selectedPages) {
        ...
        let sigW, sigH, x, y;
        if (t?.wFrac) {
          sigW = t.wFrac * pW; sigH = sigW / sigInfo.aspectRatio;
          x    = t.xFrac * pW; y    = pH * (1 - t.yFrac) - sigH;
        } else {
          sigW = Math.min(200, pW * 0.35); sigH = sigW / sigInfo.aspectRatio;
          x    = pW - sigW - 28;           y    = 28;
        }
        x = Math.max(0, Math.min(x, pW - sigW));
        y = Math.max(0, Math.min(y, pH - sigH));
        page.drawImage(sigImg, { x, y, width: sigW, height: sigH });
      }

      const bytes   = await pdfDoc.save();
      const blobUrl = URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' }));
      Object.assign(document.createElement('a'), { href: blobUrl, download: 'signed.pdf' }).click();
  ...
  const FileStrip = ({ className = '' }) => (
    <div className={className} style={{
      ...
      border: `1.5px solid ${palette.border.default}`,
      fontSize: '0.82rem',
    }}>
      <i className="bi bi-file-earmark-pdf-fill" style={{ color: '#ef4444', fontSize: '1.05rem', flexShrink: 0 }} />
      <span style={{ flex: 1, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: palette.text.primary }}>
        {pdfFile?.name}
      </span>
      {pages.length > 0 && (
        <span style={{ color: palette.text.muted, flexShrink: 0, fontSize: '0.76rem' }}>
  ...
  const NavBtn = ({ onClick, disabled, children }) => (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      background: disabled ? palette.surface.overlay : palette.primary,
      color: disabled ? palette.text.muted : '#fff',
      fontWeight: 700, fontSize: '0.88rem', cursor: disabled ? 'not-allowed' : 'pointer',
      opacity: disabled ? 0.45 : 1,
      transition: 'all 0.15s', flexShrink: 0,
      boxShadow: disabled ? 'none' : `0 4px 14px ${palette.primary}44`,
    }}>
      {children}
    </button>
  );
  ...
  return (
    <>
      <SEO
        ...
        description="Add your typed or hand-drawn signature to any PDF. 100% browser-based."
        keywords="sign PDF online, add signature to PDF, PDF signature, digital signature PDF free"
      />
      ...
      <Container className="py-4 py-md-5" style={{ maxWidth: 1160 }}>
        ...
        {!pdfFile && (
          <div style={{ maxWidth: 660, margin: '0 auto' }}>
            <DropZone onFiles={handleFiles} accept="application/pdf" multiple={false}
          ...
          <Row className="g-4 align-items-start">
            ...
            <Col xs={12} md={5} lg={4}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div className="settings-panel">
                  <StepHeader n={1} done={step1Done}
                    title="Create Your Signature"
                    subtitle="Type or draw · set colour and pen weight" />
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                      {COLORS.map(c => {
                        ...
                        return (
                          <button key={c.id} onClick={() => { setSigColor(c.id); setCustomColor(''); setShowColorPicker(false); }}
                            title={c.label}
                            style={{
                              width: 32, height: 32, borderRadius: '50%', background: c.hex,
                              border: 'none', cursor: 'pointer', transition: 'all 0.14s',
                              position: 'relative', flexShrink: 0,
                              boxShadow: active ? `0 0 0 2.5px #fff, 0 0 0 4.5px ${palette.primary}` : '0 1px 4px rgba(0,0,0,0.22)',
                      ...
                      <button
                        ...
                        style={{
                          ...
                          boxShadow: customColor
                            ? `0 0 0 2.5px #fff, 0 0 0 4.5px ${palette.primary}`
                            : showColorPicker
                              ? `0 0 0 2.5px #fff, 0 0 0 4.5px ${palette.primary}`
                              : '0 1px 4px rgba(0,0,0,0.22)',
                          transform: (customColor || showColorPicker) ? 'scale(1.18)' : 'scale(1)',
                          outline: 'none',
                        }}>
                        {!customColor && (
                          <i className="bi bi-eyedropper" style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', color: '#fff', filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.6))' }} />
                        )}
                        {customColor && (
                          <i className="bi bi-check" style={{ color: '#fff', fontSize: '0.82rem', position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.5))' }} />
                        )}
                      </button>
                    </div>

                    {/* ── Colour picker modal (portal, see ColorPickerModal above) ── */}
                    {showColorPicker && (
                      <ColorPickerModal
                        colorHex={colorHex}
                ...
                {/* STEP 2 */}
                <div className="settings-panel">
                  <StepHeader n={2} done={step2Done}
                    title="Pages to Sign"
                    subtitle="Tap to select · eye = currently previewed" />

                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <span style={{ fontSize: '0.76rem', color: palette.text.muted }}>
                      {allSelected ? 'All pages selected' : `${selectedPages.size} of ${pages.length} selected`}
                    </span>
                    <Form.Check type="switch" id="all-pages-switch"
                      label={<span style={{ fontSize: '0.76rem', fontWeight: 600 }}>All</span>}
                  ...
                  <div className="thin-scroll" style={{ maxHeight: 200, overflowY: 'auto', paddingBottom: 8 }}>
                    <Row className="g-2">
                      {pages.map((p, i) => {
                        const sel  = selectedPages.has(i);
                        const prev = previewPage === i;
                        const pos  = positionedPages.has(i);
                        return (
                          <Col key={i} xs={3}>
                            <div onClick={() => togglePage(i)} style={{
                              cursor: 'pointer', borderRadius: 8, overflow: 'hidden', position: 'relative',
                              border: sel ? `2.5px solid ${palette.primary}` : `1.5px solid ${palette.border.default}`,
                              background: sel ? `${palette.primary}08` : palette.surface.base,
                              transition: 'all 0.13s',
                              boxShadow: sel ? `0 2px 8px ${palette.primary}28` : '0 1px 3px rgba(0,0,0,0.05)',
                            }}>
                              ...
                              {/* Positioned dot */}
                              {pos && (
                                <div style={{ position: 'absolute', top: 3, left: 3, width: 12, height: 12, borderRadius: '50%', background: '#16a34a', border: '1.5px solid #fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                  <i className="bi bi-check" style={{ color: '#fff', fontSize: '0.4rem' }} />
                                </div>
                              )}
            ...
            <Col xs={12} md={7} lg={8}>
              <div style={{ position: 'sticky', top: 76 }}>
                ...
                <div className="settings-panel">
                  <StepHeader n={3} done={false}
                    title="Place Your Signature"
                    subtitle="Drag to move · corner handle or pinch to resize" />

                  {/* PDF preview canvas */}
                  <SignatureCanvas
                    key={previewPage}
                    pageImgUrl={previewPageUrl}
                    sigImgUrl={sigInfo?.dataUrl ?? ''}
                    sigAspectRatio={sigInfo?.aspectRatio ?? 4}
                    transformRef={sigTransformRef}
                    initialTransform={pageTransformsRef.current[previewPage]}
                  />

                  {/* ── Page navigation — BELOW canvas, large buttons ── */}
                  {pages.length > 1 && (
                    <div style={{
                      ...
                      <NavBtn
                        onClick={() => switchPage(previewPage - 1)}
                      ...
                      <div style={{ flex: 1, textAlign: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: 5, justifyContent: 'center', flexWrap: 'wrap' }}>
                          {selectedPages.has(previewPage) ? (
                            <span style={{ fontSize: '0.68rem', padding: '2px 9px', borderRadius: 20, background: `${palette.primary}15`, color: palette.primary, fontWeight: 700 }}>
                              <i className="bi bi-pen me-1" />Signing
                            </span>
                          ) : (
                            <span style={{ fontSize: '0.68rem', padding: '2px 9px', borderRadius: 20, background: palette.surface.base, color: palette.text.muted, fontWeight: 600, border: `1px solid ${palette.border.default}` }}>
                              Not selected
                            </span>
                          )}
                          {positionedPages.has(previewPage) && (
                            <span style={{ fontSize: '0.68rem', padding: '2px 9px', borderRadius: 20, background: '#16a34a18', color: '#16a34a', fontWeight: 700 }}>
                              <i className="bi bi-check-circle me-1" />Positioned
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div style={{ marginTop: 16 }}>
                    <Button variant="primary" size="lg" className="w-100"
                      onClick={applySignature} disabled={isProcessing || !canDownload}
                      style={{ borderRadius: 12, fontWeight: 700, fontSize: '1rem', padding: '13px 0', boxShadow: canDownload ? `0 6px 22px ${palette.primary}48` : 'none', transition: 'box-shadow 0.3s' }}>
                      {isProcessing ? (
                        <><Spinner size="sm" className="me-2" />Applying Signature…</>
                      ) : (
                        <>
                          <i className="bi bi-cloud-download me-2" />Apply &amp; Download
                          {selectedPages.size > 1 && <Badge bg="light" text="dark" className="ms-2" pill>{selectedPages.size} pages</Badge>}
                        </>
                      )}
                    </Button>

                    {!step1Done && (
                      <p style={{ textAlign: 'center', color: palette.text.muted, fontSize: '0.77rem', marginTop: 8, marginBottom: 0 }}>
                        <i className="bi bi-arrow-left me-1" />Create your signature in Step 1 to continue
                      </p>
                    )}
                  </div>
                </div>
