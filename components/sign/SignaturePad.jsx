...
const drawToCanvas = (text, fontFamily, colorHex, fontSize, fontWeight, penWeight) => {
  ...
  const padX    = 20 + extra, textY = fontSize + extra;
  ...
  ctx.font  = `${fontWeight} ${fontSize}px ${fontFamily}`;
  if (strokeW > 0.3) { ctx.strokeStyle = colorHex; ctx.lineWidth = strokeW * 2; ctx.lineJoin = 'round'; ctx.lineCap = 'round'; ctx.strokeText(text, padX, textY); }
  ctx.fillStyle = colorHex; ctx.fillText(text, padX, textY);
...
const SignaturePad = ({ colorHex, onChange }) => {
  ...
  useEffect(() => { FONTS_FLAT.forEach(f => document.fonts.load(`${f.weight} ${EXPORT_PX}px ${f.family}`).catch(() => {})); }, []);

  useEffect(() => {
    if (mode !== 'type') return;
    const text = sigText.trim() || 'Your Signature';
    const { canvas: src, dataUrl } = drawToCanvas(text, activeFont.family, colorHex, PREVIEW_PX, activeFont.weight, penWeight);
    const cv = previewRef.current;
    if (cv) { cv.width = src.width; cv.height = src.height; cv.getContext('2d').drawImage(src, 0, 0); }
    cbRef.current({ dataUrl, aspectRatio: src.width / src.height, getHighRes: () => buildSigCanvas(text, activeFont.family, colorHex, EXPORT_PX, activeFont.weight, penWeight).then(r => r.dataUrl) });
  }, [mode, sigText, sigFont, penWeight, colorHex]); // eslint-disable-line
  ...
  const getPos = (e) => {
    const cv = drawRef.current, rect = cv.getBoundingClientRect(), src = e.touches ? e.touches[0] : e;
  ...
  const onDrawStart = (e) => {
    ...
    const cv = drawRef.current, scale = cv.width / cv.getBoundingClientRect().width, pos = getPos(e);
    isDrawing.current = true; lastPos.current = pos;
    const ctx = cv.getContext('2d');
    ctx.beginPath(); ctx.arc(pos.x, pos.y, (brushSize * scale) / 2, 0, Math.PI * 2); ctx.fillStyle = colorHex; ctx.fill();
  };

  const onDrawMove = (e) => {
    e.preventDefault();
    if (!isDrawing.current || !lastPos.current) return;
    const cv = drawRef.current, scale = cv.width / cv.getBoundingClientRect().width, pos = getPos(e), ctx = cv.getContext('2d');
    ctx.beginPath(); ctx.moveTo(lastPos.current.x, lastPos.current.y); ctx.lineTo(pos.x, pos.y);
    ctx.strokeStyle = colorHex; ctx.lineWidth = brushSize * scale; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.stroke();
    lastPos.current = pos;
  };
    ...
    <div>
      <div style={{ display: 'flex', borderRadius: 12, overflow: 'hidden', border: `1.5px solid ${palette.border.default}`, marginBottom: 28 }}>
        {[{ id: 'type', label: 'Type', icon: 'bi-keyboard' }, { id: 'draw', label: 'Draw', icon: 'bi-pen-fill' }].map(m => (
          <button key={m.id} onClick={() => setMode(m.id)} style={{ flex: 1, padding: '13px 0', border: 'none', cursor: 'pointer', transition: 'background 0.18s, color 0.18s', background: mode === m.id ? palette.primary : 'transparent', color: mode === m.id ? '#fff' : palette.text.muted, fontWeight: 700, fontSize: '0.92rem' }}>
      ...
      {mode === 'type' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div>
            ...
            <Form.Control type="text" placeholder="e.g. John Smith" value={sigText} onChange={e => setSigText(e.target.value)} maxLength={60} style={{ fontSize: '1rem', borderRadius: 10, padding: '10px 14px' }} />
          </div>
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <label className="fw-semibold small mb-0"><i className="bi bi-pen me-1" style={{ color: palette.text.muted }} />Pen Size</label>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, fontSize: '0.68rem', color: palette.text.muted }}><span>Hairline</span><span>Brush</span></div>
          </div>
          ...
          <div>
            ...
            <Accordion defaultActiveKey={activeCatLabel} flush>
              {FONT_CATEGORIES.map(cat => {
                ...
                return (
                  <Accordion.Item key={cat.label} eventKey={cat.label} style={{ border: `1.5px solid ${hasSel ? palette.primary + '44' : palette.border.default}`, borderRadius: 10, marginBottom: 8, overflow: 'hidden' }}>
                    <Accordion.Header>
                    ...
                    <Accordion.Body style={{ padding: '8px 10px 12px', background: `${palette.surface.overlay}80` }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {cat.fonts.map(font => (
                          <div key={font.id} onClick={() => setSigFont(font.id)} style={{ padding: '10px 14px', borderRadius: 9, cursor: 'pointer', transition: 'all 0.13s', border: sigFont === font.id ? `2px solid ${palette.primary}` : `1.5px solid transparent`, background: sigFont === font.id ? `${palette.primary}0a` : palette.surface.base, display: 'flex', alignItems: 'center', gap: 10, boxShadow: sigFont === font.id ? `0 2px 8px ${palette.primary}20` : '0 1px 3px rgba(0,0,0,0.05)' }}>
                            <span style={{ fontFamily: font.family, fontWeight: font.weight, fontSize: '1.5rem', color: colorHex, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.35 }}>{sigText.trim() || 'Your Signature'}</span>
                            <span style={{ fontSize: '0.62rem', color: palette.text.muted, flexShrink: 0 }}>{font.label}</span>
                            <i className={`bi ${sigFont === font.id ? 'bi-check-circle-fill text-primary' : 'bi-circle'}`} style={{ flexShrink: 0, fontSize: '0.9rem', opacity: sigFont === font.id ? 1 : 0.22 }} />
                          </div>
                        ))}
                      </div>
                    </Accordion.Body>
      ...
      {mode === 'draw' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <label className="fw-semibold small mb-0"><i className="bi bi-brush me-1" style={{ color: palette.text.muted }} />Brush Size</label>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, fontSize: '0.68rem', color: palette.text.muted }}><span>Fine</span><span>Thick</span></div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Button variant="outline-danger" size="sm" onClick={clearDraw} style={{ borderRadius: 8 }}><i className="bi bi-arrow-counterclockwise me-1" />Clear</Button>
            {hasDrawn && <span style={{ fontSize: '0.8rem', color: '#16a34a', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}><i className="bi bi-check-circle-fill" />Signature captured</span>}
          </div>
        </div>
      )}
    </div>
