  ...
  for (const part of parts) {
    if (part.includes('-')) {
      ...
      if (Number.isInteger(a) && Number.isInteger(b) && a >= 1 && b <= total && a <= b)
        groups.push({ label: `pages_${a}-${b}`, indices: Array.from({ length: b - a + 1 }, (_, i) => a - 1 + i) });
    } else {
      const n = Number(part);
      if (Number.isInteger(n) && n >= 1 && n <= total) groups.push({ label: `page_${n}`, indices: [n - 1] });
    }
  }
  return groups;
};
...
export default function SplitPDFClient() {
  ...
  const handleFiles = async (files) => {
    ...
    setError(''); setSuccess(''); setPdfFile(file);
    try {
      const pdf = await PDFDocument.load(await file.arrayBuffer());
      setPageCount(pdf.getPageCount());
    } catch (err) { setError('Failed to read PDF: ' + err.message); setPdfFile(null); }
  };
  ...
  const splitPDF = async () => {
    ...
    try {
      ...
      if (splitMode === 'all') {
        ...
        if (groups.length === 0) { setError(`Invalid ranges. Use format like: 1-3, 5, 7-9  (PDF has ${pageCount} pages)`); setIsProcessing(false); return; }
      }
      ...
      <Container className="py-4 py-md-5" style={{ maxWidth: 680 }}>
        ...
        {pdfFile && pageCount > 0 && (
          <div className="settings-panel">
            ...
            <Form.Check type="radio" id="split-all" label={`Extract each page as a separate PDF (${pageCount} files)`} checked={splitMode === 'all'} onChange={() => setSplitMode('all')} className="mb-3" />
            <Form.Check type="radio" id="split-range" label="Split by custom page ranges" checked={splitMode === 'range'} onChange={() => setSplitMode('range')} className="mb-3" />
            {splitMode === 'range' && (
              <Form.Group className="mb-4 ms-4">
                <Form.Control type="text" placeholder="e.g. 1-3, 5, 7-9" value={rangeInput} onChange={e => setRangeInput(e.target.value)} />
                <Form.Text className="text-muted">Each entry becomes a separate PDF. This PDF has {pageCount} pages. Example: <code>1-3, 5, 7-9</code> creates 3 files.</Form.Text>
              </Form.Group>
            )}
            <div className="text-center mt-2">
              <Button variant="primary" size="lg" onClick={splitPDF} disabled={isProcessing} className="px-5">
                {isProcessing ? <><Spinner size="sm" className="me-2" />Splitting…</> : <><i className="bi bi-archive me-2"></i>Split &amp; Download ZIP</>}
              </Button>
            </div>
          </div>
        )}
